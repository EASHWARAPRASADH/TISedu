    ...
    <div class="sa-card-header">
        ...
        <div class="sa-card-tools">
            <form action="{{ route('superadmin.subscriptions.index') }}" method="GET" style="display: flex; gap: 8px;">
                <input type="text" name="search" class="sa-form-control sa-form-control-sm" placeholder="Search schools..." value="{{ request('search') }}">
        ...
        <table class="sa-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span class="sa-badge" style="background: rgba(102,126,234,0.1);">{{ $school->plan_type ?: 'Standard' }}</span>
                    </td>
                    <td>
                        @if($expiry)
                            {{ $expiry->format('d M, Y') }}
                    ...
                    <td>
                        <button class="sa-btn sa-btn-outline sa-btn-sm" onclick="openExtendModal('{{ $school->id }}', '{{ $school->school_name }}', '{{ $school->ending_date }}', '{{ $school->active_status }}', '{{ $school->plan_type }}')">
                            <i class="fas fa-edit"></i> Manage
                        </button>
                        <form action="{{ route('superadmin.subscriptions.toggle', $school->id) }}" method="POST" style="display:inline;">
                            @csrf
                            <button type="submit" class="sa-btn {{ $school->active_status == 1 ? 'sa-btn-danger' : 'sa-btn-success' }} sa-btn-sm">
